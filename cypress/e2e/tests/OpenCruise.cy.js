...
describe("TEST WEB OPEN CRUISE", () => {
  ...
  it(
    "TEST01 ** Se Connecter avec un Compte Admin Valide **",
    { tags: "@smoke" },
    () => {
      const loginStep = new LoginStep();
      cy.url().then((url) => {
        if (url.includes("-ok")) {
          // L'URL contient "-ok", effectuez les actions nécessaires
          loginStep.setLogin(Cypress.env("USERNAME"), Cypress.env("PASSWORD"));
          loginStep.checkMessage("Bienvenue ADMIN TEST");
        } else {
          // L'URL ne contient pas "-ok", continuez avec l'URL actuelle
          loginStep.setLogin(Cypress.env("USERNAME1"), Cypress.env("PASSWORD"));
          loginStep.checkMessage("Bienvenue admin2 admin2");
        }
  ...
  it("TEST03 ** Création et Activation d'un Compte Particulier **", () => {
    ...
    cy.get("@users").then((data) => {
      ...
      cy.writeFile(
        ...
        "utf8"
      ); // Write user data to "userPart.json"
      cy.wait(2000);
      const loginStep = new LoginStep();
  ...
  it("TEST04 ** Création et Activation d'un Compte Professionnel **", () => {
    ...
    cy.get("@users").then((data) => {
      ...
      cy.writeFile(
        ...
        JSON.stringify(data, null, 2),
        "utf8"
      ); // Write user data to "userPro.json"
      cy.wait(1000);
      const loginStep = new LoginStep();
      cy.log(`Se Connecter en tant que Admin`);
      cy.url().then((url) => {
        if (url.includes("-ok")) {
          ...
          // L'URL ne contient pas "-ok", continuez avec l'URL actuelle
          loginStep.setLogin(Cypress.env("USERNAME1"), Cypress.env("PASSWORD"));// Log in as admin
        }
        const stepGestionCompte = new StepGestionCompte();
        stepGestionCompte.ActiveCompte(data); // Activate the account
        cy.xpath(`//tr[contains(.,'${data.username}')]`).screenshot(); // Take a screenshot
  ...
  it("TEST05 ** Création d'un Compte Particulier avec un Compte Active **", () => {
    const stepSouscription = new StepSouscription();
    cy.log(
      "**********   Inscription Compte Particulier Avec Compte Existant    ************"
    );
